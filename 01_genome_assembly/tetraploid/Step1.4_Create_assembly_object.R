...
#### Get the distribution of coverages, infer 1X, 2X, 3X, and 4X boundaries
...
#read unitig lengths
f <- '/filer-dg/agruppen/dg2/fengj/hbulbosum_genome/genome_assembly/A40/A40_hifiasm.fasta.fai'
...
#read Hi-C map
dir <- '/filer-dg/agruppen/dg2/fengj/hbulbosum_genome/genome_assembly/A40/hic/hicreads'
...
saveRDS(assembly, file="A40_assembly.Rds")

##
# Check for and correct chimeras 
##

readRDS('A40_assembly.Rds') -> assembly
...
plot_chimeras(assembly=assembly, scaffolds=b,  species="barley", refname="Morex HC genes", autobreaks=F, mbscale=1, file="A40_assembly_all5_contig.pdf", cores=50)
...
i=246
ss[i]$scaffold -> s 
assembly$cov[s, on='scaffold'][bin >= 4e6 & bin <= 6e6][order(r)][1, .(scaffold, bin)] -> b

i=761
ss[i]$scaffold -> s 
rbind(b, assembly$cov[s, on='scaffold'][bin >= 4e6 & bin <= 5e6][order(r)][1, .(scaffold, bin)])->b

i=1063
ss[i]$scaffold -> s 
rbind(b, assembly$cov[s, on='scaffold'][bin >= 0.5e6 & bin <= 1e6][order(r)][1, .(scaffold, bin)])->b

i=1193
ss[i]$scaffold -> s 
rbind(b, assembly$cov[s, on='scaffold'][bin >= 1e6 & bin <= 1.5e6][order(r)][1, .(scaffold, bin)])->b



setnames(b, "bin", "br")
plot_chimeras(assembly=assembly, scaffolds=b, br=b, species="barley", refname="MorexV3 genes",  mbscale=1,
         file="A40_assembly_chimeras_final.pdf", cores=30)

#implement the correction
break_scaffolds(b, assembly, prefix="contig_corrected_v1_", slop=1e4, cores=30, species="barley") -> assembly_v2

#save the object
saveRDS(assembly_v2, file="A40_assembly_v2.Rds")
